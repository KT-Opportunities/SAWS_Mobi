...
export class CentralInterioPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.APIService.GetSourceAviationFolderFilesList('aerosport', 24).subscribe(
      (data) => {
        this.centralInterio = data.filter((item: any) =>
          item.filename.toLowerCase().includes('cint')
        );
        console.log('Central Interio:', this.centralInterio);
        this.CloudCover = this.centralInterio.filter(
        ...
        this.ConvectiveCloudBase = this.centralInterio.filter(
          (item: { filename: string }) =>
        ...
        this.WindArray = this.centralInterio.filter(
          (item: { filename: string }) =>
        ...
        this.ThermalArray = this.centralInterio.filter(
          (item: { filename: string }) =>
        ...
        this.TemperatureArray = this.centralInterio.filter(
          (item: { filename: string }) =>
  ...
  getFilteredItemsThermal() {
    if (this.selectedOption3 === 'Normal') {
      return this.TemperatureArray.filter((item: any) => item.startsWith('t'));
    } else if (this.selectedOption3 === 'Middle') {
      return this.TemperatureArray.filter((item: any) => item.startsWith('m'));
    } else {
      return this.TemperatureArray;
    }
  }

  getFilteredItemsTemperature() {
    if (this.selectedOption3 === 'Normal') {
      return this.TemperatureArray.filter((item: any) => item.startsWith('n'));

    }else if (this.selectedOption3 === 'Middle') {
      return this.TemperatureArray.filter((item: any) => item.startWidth('m'));
    }
    else  {
      return this.TemperatureArray;
    }
  ...
  filterItems() {
    if (this.selectedOption === 'Low') {
      this.filteredConvectiveCloudBase = this.ConvectiveCloudBase.filter((item: any) =>
        item.startsWith('cb00')
      );
    } else if (this.selectedOption === 'Middle') {
      this.filteredConvectiveCloudBase = this.ConvectiveCloudBase.filter((item: any) =>
        item.startsWith('cb12')
      );
    } else if (this.selectedOption === 'High') {
      this.filteredConvectiveCloudBase = this.ConvectiveCloudBase.filter((item: any) =>
        item.startsWith('cb23')
      );
    } else {
