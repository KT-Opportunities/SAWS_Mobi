...
export class ColorCodedTafComponent implements OnInit, OnDestroy {
  ...
  private loadTAFs() {
    ...
    const processResponse = (response: FileData[]) => {
      this.TAFArray.push(...response);
    ...
    // Fetch both folders
    this.apiService.GetSourceTextFolderFilesTime('taffc', 72).subscribe(response => {
      processResponse(response);
      this.groupTAFsByProvince(allTAFs);
      this.spinner.hide();
    });

    this.apiService.GetSourceTextFolderFilesTime('tafft', 72).subscribe(response => {
      processResponse(response);
      this.groupTAFsByProvince(allTAFs);
  ...
  private groupTAFsByProvince(allTAFs: { [icao: string]: string }) {
    const grouped: { [province: string]: string[] } = {};

    for (const province in this.provinces) {
      grouped[province] = [];
      this.provinces[province].forEach(station => {
        grouped[province].push(allTAFs[station] || `No data for ${station}`);
      });
    this.viewModes[province] = 'color';
    }

    this.groupedTAFs = grouped;
    this.filteredGroupedTAFs = { ...grouped }; // Initialize filtered view for search
  }
 ...
 onSearch(event: Event) {
  ...
  if (!query) {
    ...
    this.filteredGroupedTAFs = { ...this.groupedTAFs };
    return;
  }

  const filtered: { [province: string]: string[] } = {};

  // Only include provinces that have matching TAFs
  for (const province of Object.keys(this.groupedTAFs)) {
    const matches = this.groupedTAFs[province].filter(taf =>
      taf.toUpperCase().includes(query)
    );
    if (matches.length > 0) {
      filtered[province] = matches;
    }
  }

  this.filteredGroupedTAFs = filtered;
}
  ...
  formatTafWithColors(content: string): SafeHtml {
    if (!content) return '';
    let formatted = content
      .replace(/TEMPO/g, `<span style="color: #4169E1; font-weight: bold;">TEMPO</span>`)
      .replace(/\bVRB\d{2}KT\b/g, `<span style="color: #4169E1;">$&</span>`);
